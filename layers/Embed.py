...
class EdgeEmbedding(nn.Module):
    def __init__(self, n_vocab, enc_emb, enc_dim, d_model, pretrained=None, dropout=0.1):
        super(EdgeEmbedding, self).__init__()
        if pretrained is not None:
            self.edge_embedding = nn.Embedding.from_pretrained(pretrained, freeze=True)
        else:
            self.edge_embedding = nn.Embedding(n_vocab, enc_emb)
        self.mlp = nn.Linear(enc_emb + enc_dim, d_model)
